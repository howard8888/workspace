...
class IntegratedRightingCoreV1:
    ...
    def __init__(
        ...
        if feeding_detail_profile is not None:
            if not isinstance(feeding_detail_profile, FeedingDetailProfileV1):
    ...
    def stop(self, reason: str) -> None:
        ...
        if self.suckle is not None:
            self.suckle.cancel()
        if self.extraction_outcomes is not None:
            self.extraction_outcomes.close(reason=self._fault)
        if self.suckle_outcomes is not None:
            self.suckle_outcomes.close(reason=self._fault)
    ...
    def run_cycle(
        ...
        if not isinstance(suckle_intervals, tuple) or len(suckle_intervals) > 16 or any(
                not isinstance(item, SuckleIntervalEvidenceV1) for item in suckle_intervals):
    ...
    def _run_cycle(
        ...
        if suckle_owner is not None:
            suckle_results = suckle_owner.consume_intervals(suckle_intervals, cutoff_tick=tick)
            for outcome in suckle_results:
                self.trace.append("hierarchy_suckle_outcome", "original Suckle claim compared; not latch proof, milk or causal credit",
                                  cycle_id=cycle, details={"pnm_id": outcome.claim.preview.pnm.pnm_id, "status": outcome.status,
                                                          "command_intervals": outcome.command_intervals,
                                                          "event_tick": outcome.evidence.feedback.event_tick
                                                          if outcome.evidence is not None else None})
        ...
        seeking_results: tuple[SeekNippleOutcomeV1, ...] = ()
        if seeking_owner is not None:
            seeking_results = seeking_owner.consume_intervals(seeking_intervals, cutoff_tick=tick)
            for seeking_outcome in seeking_results:
        ...
        if self.feeding_detail is not None:
            ...
            if self.suckle is not None:
                ...
                support_pending = (self.stand_follow_enabled and previous_support_task is not None
                                   and previous_support_task.status != "completed")
                extraction_blocked = any(
                    item.current.target.kind is not SensorimotorTargetKindV1.ORAL_EXTRACTION
                    or item.current is not self.suckle.authorized_target for item in self.cognition.mapper.reservations(at_tick=tick)
            ...
            if seeking_attention is not None:
                ...
                for seeking_request in seeking_requests:
                    self.trace.append("hierarchy_seeking_outcome_request", "seeking discrepancy requested current source consideration, not an action",
                                      cycle_id=cycle, phase=CyclePhase.UPDATE_OUTCOMES.name,
                                      details={"request_id": seeking_request.request_id, "significance": seeking_request.significance,
                                               "pnm_id": seeking_request.outcome.claim.preview.pnm.pnm_id,
                                               "expires_at_tick": seeking_request.expires_at_tick})
            ...
            self.trace.append("hierarchy_feeding_source", "feeding-detail evidence updated; nomination, when present, is not an oral action",
                              cycle_id=cycle, phase=CyclePhase.UPDATE_OUTCOMES.name,
                              details={"association": feeding_source.association_status,
                                       "recognition": feeding_source.recognition_status,
                                       "detail_localized": feeding_source.detail_position is not None,
                                       "candidate": feeding_candidate is not None,
                                       "event_tick": feeding_source.maternal.visual.event_tick,
                                       "feeding_motor_authority": False})
        ...
        # Retirement is not the command deadline: the lower executor may still
        # consume L-B's final in-lease acquisition in its original bounded window.
        # All live-permission readers continue to enforce expires_at_tick.
        if self.suckle is None or not self.suckle.awaiting_final_extraction_evidence(at_tick=tick):
        ...
        if extraction_attention is not None:
            ...
            selected = self.cognition.select_prepared(prepared, maternal_attention=maternal_attention, seeking_attention=seeking_attention)
        else:
            selected = (self.cognition.select_prepared(prepared, maternal_attention=maternal_attention)
        ...
        if suckle_allocation is not None:
            ...
            self.trace.append("hierarchy_suckle_outcome_allocation", "Navigation allocation: interpretation is not an IP reapplication",
                              cycle_id=cycle, phase=CyclePhase.FOCAL_COMMITMENT.name,
                              details={"kind": suckle_allocation.kind,
                                       "request_id": suckle_interpretation.request.request_id if suckle_interpretation is not None else None,
                                       "status": suckle_interpretation.status if suckle_interpretation is not None else None,
                                       "outcome_rank": feeding_bid.prediction_or_envelope_failure_rank if feeding_bid is not None else 0})
        seeking_allocation = selected.seeking_outcome_allocation
        if seeking_allocation is not None:
            seeking_interpretation = seeking_allocation.interpretation
            self.trace.append("hierarchy_seeking_outcome_allocation", "one seeking focal allocation; no simultaneous primitive response",
                              cycle_id=cycle, phase=CyclePhase.FOCAL_COMMITMENT.name,
                              details={"kind": seeking_allocation.kind,
                                       "request_id": seeking_interpretation.request.request_id if seeking_interpretation is not None else None,
                                       "status": seeking_interpretation.status if seeking_interpretation is not None else None,
                                       "outcome_rank": feeding_bid.prediction_or_envelope_failure_rank if feeding_bid is not None else 0})
        ...
        suckle_registration = None
        if suckle_owner is not None and isinstance(application, SuckleApplicationV1) and proposal is not None:
            suckle_registration = suckle_owner.register(application, proposal, tuple(item.current for item in reservations))
            self.trace.append("hierarchy_suckle_claim", "original Suckle forecast reconciled before handoff", cycle_id=cycle,
                              details={"pnm_id": suckle_registration.preview.pnm.pnm_id,
                                       "endpoint_event_tick": suckle_registration.due_tick,
                                       "authorization": suckle_registration.authorization_status,
                                       "compatible": ",".join(suckle_registration.compatible_relations),
                                       "unevaluable": ",".join(suckle_registration.unevaluable_relations)})
...
class IntegratedRightingTrialV1:
    ...
    def __init__(
        ...
        self._suckle_intervals: list[SuckleIntervalEvidenceV1] = []
        ...
        if oral_extraction_profile is not None and (suckle_profile is None or oral_seal_profile is None):
    ...
    @property
    def observer_oral_extraction_body(self) -> OralExtractionWorldStateV1 | None:
        """Read physical transfer for external review only; no cognitive milk inference."""
    ...
    def retained_counts(self) -> dict[str, int]:
        ...
        return {
            ...
            **(self.core.feeding_detail.suckle_outcome_attention.retained_counts()
    ...
    def snapshot(self) -> dict[str, object]:
        ...
        return {
            ...
            **({"suckle_extraction": extraction.as_dict()} if extraction is not None else {}),
            ...
            **({"suckle_pending_claims": [item.as_dict() for item in self.core.suckle_outcomes.pending()],
                "suckle_outcome_history": [item.as_dict() for item in self.core.suckle_outcomes.history()]}
            ...
            if result.reservations:
                ...
                if self.core.extraction_outcomes is not None:
                    ...
                    if extraction_claim is not None:
                        self.core.extraction_outcomes.installed(extraction_claim, at_tick=self.tick)
                    else:
                        self.core.extraction_outcomes.end_execution(at_tick=self.tick, reason="replacement")
                if self.core.suckle_outcomes is not None:
                    suckle_claim = result.suckle_correspondence.registration if result.suckle_correspondence is not None else None
                    if suckle_claim is not None:
                        self.core.suckle_outcomes.installed(suckle_claim, at_tick=self.tick)
                    else:
                        self.core.suckle_outcomes.end_execution(at_tick=self.tick, reason="replacement")
                ...
                if self.core.suckle_outcomes is not None:
                    self.core.suckle_outcomes.end_execution(at_tick=self.tick)
                if self.core.seeking_outcomes is not None:
    ...
    def advance_lower(self) -> SensorimotorStepV1:
        ...
        if (self.core.suckle_outcomes is not None or self.core.extraction_outcomes is not None) and len(self._suckle_intervals) >= 16:
            self._stop("suckle_ingress_overflow_before_physical_step")
            raise OverflowError("sixteen Suckle intervals await C2; no further physical step")
        ...
        try:
            ...
            if (self.core.maternal_outcomes is not None or self.core.seeking_outcomes is not None
                ...
                if self.core.suckle_outcomes is not None or self.core.extraction_outcomes is not None:
                    self._suckle_intervals.append(SuckleIntervalEvidenceV1(tick, command, result.reports, delivered, tuple(observations)))
            self.core.trace.append(
