...
from nca8_rest_demo import REST_CASES_V1, render_rest_v1, run_rest_v1
from nca8_sustained_feeding_demo import SUSTAINED_FEEDING_CASES_V1, render_sustained_feeding_v1, run_sustained_feeding_v1
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    allowed = (REST_CASES_V1 if args.rest else SUSTAINED_FEEDING_CASES_V1 if args.sustained_feeding else EXTRACTION_LEARNING_CASES_V1 if args.suckle_extraction_learning else
    ...
    if args.rest:
        rest_results = tuple(run_rest_v1(case) for case in cases)
        print(json.dumps([item.as_dict() for item in rest_results], sort_keys=True, allow_nan=False) if args.json else
              "\n\n".join(render_rest_v1(item, detail=args.detail) for item in rest_results))
        return int(any(item.review_status != "PASS" for item in rest_results))
    if args.sustained_feeding:
        sustained_results = tuple(run_sustained_feeding_v1(case) for case in cases)
        print(json.dumps([item.as_dict() for item in sustained_results], sort_keys=True, allow_nan=False) if args.json else
              "\n\n".join(render_sustained_feeding_v1(item, detail=args.detail) for item in sustained_results))
        return int(any(item.review_status != "PASS" for item in sustained_results))
