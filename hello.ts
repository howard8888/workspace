...
interface AppOptions {
  ...
  noIntro: boolean;
  noColor: boolean;        // override env NO_COLOR
  logo: LogoStyle | null;  // override env CCA8_LOGO
  profile: string | null;  // optional preselect (goat/chimp/human/society/planning/super or 1..7)

  logPath: string | null;  // transcript file path
}
...
const CCA8_RUNNER_VERSION = "0.8.2"; // from cca8_run.py __version__
...
function padLeft(s: string, width: number, ch: string): string {
  const ss = String(s);
  if (ss.length >= width) return ss;
  let out = "";
  for (let i = 0; i < width - ss.length; i++) out += ch;
  return out + ss;
}
...
function envGet(name: string, fallback: string): string {
  try {
    const v = process && process.env ? process.env[name] : undefined;
...
function shouldUseColor(noColorFlag: boolean): boolean {
  ...
  if (envHas("NO_COLOR")) return false;
  try {
...
const PROFILE_NARRATIVE_MULTI_BRAINS = [
  "",
...
const PROFILE_NARRATIVE_SOCIETY = [
  "",
...
const PROFILE_NARRATIVE_ADV_PLANNING = [
  "",
...
class Logger {
  ...
  write(text: string): void {
    ...
    try {
      ...
      // fallback
      try { console.log(text); } catch (_e2) { /* ignore */ }
    }
  ...
  block(text: string): void {
    ...
    this.write(text);
    if (!endsWithNewline(text)) this.write("\n");
  }
}
...
class Cli {
  ...
  close(): void {
    try { this.rl.close(); } catch (_e) { /* ignore */ }
    this.logger.close();
  }
  ...
  outBlock(text: string): void {
    this.logger.block(text);
  }

  prompt(question: string, cb: (answer: string) => void): void {
    this.rl.question(question, function(answer: string) {
      cb(String(answer));
    });
  }

  pause(cb: (answer: string) => void): void {
...
function defaultOptions(): AppOptions {
  return {
    about: false,
    version: false,
    understanding: false,
...
function parseArgs(argv: string[]): AppOptions {
  ...
  while (i < argv.length) {
    ...
    if (a === "--logo") {
      ...
      const vv = lower(trimStr(v));
      if (vv === "badge" || vv === "goat" || vv === "off") {
        opts.logo = vv as LogoStyle;
        i += 2;
        continue;
      }
      i += 1;
    ...
    if (a === "--profile") {
      const v2 = (i + 1 < argv.length) ? String(argv[i + 1]) : "";
      if (!isEmpty(v2)) {
    ...
    if (a === "--log") {
      const lp = (i + 1 < argv.length) ? String(argv[i + 1]) : "";
      if (!isEmpty(lp)) {
...
function logoFromEnvOrDefault(explicit: LogoStyle | null): LogoStyle {
  if (explicit) return explicit;
  const v = lower(trimStr(envGet("CCA8_LOGO", "badge")));
  if (v === "badge" || v === "goat" || v === "off") return v as LogoStyle;
  return "badge";
}

function printAsciiLogo(cli: Cli, style: LogoStyle, colorEnabled: boolean): void {
  if (style === "off") return;

  let art = (style === "goat") ? ASCII_GOAT : ASCII_BADGE;
...
function printHeader(cli: Cli, st: RuntimeState): void {
  cli.out("");
  cli.out("");
...
function getEntrypoint(): string {
  try {
    const p = process && process.argv && process.argv.length >= 2 ? String(process.argv[1]) : "(unknown)";
    return p;
  } catch (_e) {
    return "(unknown)";
  }
...
function printAbout(cli: Cli): void {
  cli.out("");
...
function printVersion(cli: Cli): void {
  cli.out("cca8_demo.ts v" + DEMO_VERSION);
  cli.out("based on cca8_run.py v" + CCA8_RUNNER_VERSION);
}

function printUnderstanding(cli: Cli): void {
  cli.outBlock(UNDERSTANDING);
}

function printQuickTour(cli: Cli): void {
  cli.outBlock(QUICK_TOUR);
}

function printTutorialMenu(cli: Cli): void {
  cli.out("");
...
function showReadmeHint(cli: Cli): void {
...
function normalizeProfileInput(s: string): string {
  const x = lower(trimStr(s));
  ...
  if (x === "3") return "human";
  if (x === "4") return "multi";
  if (x === "5") return "society";
  if (x === "6") return "planning";
  if (x === "7") return "super";

  // name shortcuts
  if (x === "goat" || x === "mountain_goat" || x === "mountaingoat") return "goat";
  if (x === "chimp" || x === "chimpanzee") return "chimp";
  if (x === "human") return "human";
  if (x === "multi" || x === "multi_brain" || x === "multibrains" || x === "multiple_brains") return "multi";
  if (x === "society" || x === "agents" || x === "multi_agents" || x === "multiagent") return "society";
  if (x === "planning" || x === "adv" || x === "combinatorial") return "planning";
  if (x === "super" || x === "asi" || x === "superhuman") return "super";

  return x;
}

function profileNarrative(kind: string): string {
  if (kind === "chimp") return PROFILE_NARRATIVE_CHIMP + PROFILE_FALLBACK;
  if (kind === "human") return PROFILE_NARRATIVE_HUMAN + PROFILE_FALLBACK;
  if (kind === "multi") return PROFILE_NARRATIVE_MULTI_BRAINS + PROFILE_FALLBACK;
  if (kind === "society") return PROFILE_NARRATIVE_SOCIETY + PROFILE_FALLBACK;
  if (kind === "planning") return PROFILE_NARRATIVE_ADV_PLANNING + PROFILE_FALLBACK;
  if (kind === "super") return PROFILE_NARRATIVE_SUPER + PROFILE_FALLBACK;
  return "";
}

function promptForProfile(cli: Cli, st: RuntimeState, preselected: string | null, cb: (profileName: string) => void): void {
  // If preselected, apply it without prompting (but still show narrative if it’s not goat).
  if (preselected && !isEmpty(preselected)) {
    const kind = normalizeProfileInput(preselected);
    if (kind !== "goat" && kind !== "t") {
      const txt = profileNarrative(kind);
      if (!isEmpty(txt)) cli.outBlock(txt);
    }
  ...
  const promptText = "Please make a choice [1–7 or T | Enter = Mountain Goat]: ";
  cli.prompt(promptText, function(answer: string) {
    const raw = trimStr(answer);
    ...
    if (a === "t") {
      ...
      cli.prompt("Choose: ", function(pick: string) {
        const p = trimStr(pick);
        if (p === "1") {
          showReadmeHint(cli);
        } else if (p === "2") {
    ...
    const kind = normalizeProfileInput(raw);
    if (kind !== "goat") {
      const txt2 = profileNarrative(kind);
...
function routeAlias(cmd: string): { routed: string | null; matches: string[] } {
  const s = lower(trimStr(cmd));
  ...
  if (s.length >= MIN_PREFIX) {
    ...
    for (let i = 0; i < keys.length; i++) {
      const k = keys[i];
      if (startsWith(k, s)) matches.push(k);
    }
    if (matches.length === 1) return { routed: ALIASES[matches[0]], matches: matches };
...
function menuLoop(cli: Cli, st: RuntimeState): void {
  cli.prompt(MENU_TEXT, function(rawAnswer: string) {
    let choice = trimStr(rawAnswer);
    const lowerChoice = lower(choice);

    if (!isEmpty(choice)) {
      if (!isDigits(choice)) {
        const routed = routeAlias(choice);
    ...
    if (choice === "2") {
      printQuickTour(cli);
      cli.pause(function(ans2: string) {
    ...
    if (choice === "3") {
      printAbout(cli);
      cli.pause(function(ans3: string) {
    ...
    if (choice === "4") {
      printHeader(cli, st);
      cli.pause(function(ans4: string) {
    ...
    if (choice === "5") {
      promptForProfile(cli, st, null, function(_profile: string) {
    ...
    if (choice === "6") {
      ...
      cli.out("  3) off");
      cli.out("");
      cli.prompt("Choose (default=badge): ", function(pick: string) {
        const p = lower(trimStr(pick));
        let style: LogoStyle = "badge";
        if (p === "2" || p === "goat") style = "goat";
        else if (p === "3" || p === "off") style = "off";
        else style = "badge";
        ...
        cli.pause(function(_ans5: string) {
          menuLoop(cli, st);
        });
      });
      return;
...
function main(): void {
  ...
  const logger = new Logger(args.logPath);
  ...
  promptForProfile(cli, state, args.profile, function(_profileName: string) {
...
function getArgv(): string[] {
  try {
    const out: string[] = [];
    const a = process && process.argv ? process.argv : [];
    for (let i = 2; i < a.length; i++) out.push(String(a[i]));
    return out;
  } catch (_e) {
    return [];
  }
